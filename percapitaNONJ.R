...
us2019pop <- 328239523 - 8882190

# Cases and deaths from COVID in USA
us <- read_csv("us.csv")

us <- sqldf('select date, sum(cases) as cases, sum(deaths) as deaths from us_states where fips <> 34 group by date')
...
ranking <- sqldf(sprintf("select date, state, daily_cases_avg, daily_cases_avg_per, daily_deaths_avg, daily_deaths_avg_per, weekly_cases_per100k, weekly_deaths_per100k from covid_daily_norm where date = '%s' and state <> 'New Jersey'", refdate))
...
ggplot(data=ranking2, aes(x=weekly_cases_per100k, y=weekly_deaths_per100k)) +geom_point()
